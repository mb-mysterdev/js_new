...
function quest2() {
  let a = 0;
  array.forEach((arrayContent) => {
  	if(arrayContent.toLowerCase().indexOf("aude") >= 0) {
	  	a++;
  	}
  });
  return a
...
function quest3() {
  let a = 0;
  array.forEach((arrayContent) => {
  	if(arrayContent.charCodeAt(1) >= "A".charCodeAt(0) && arrayContent.charCodeAt(1) <= "Z".charCodeAt(0)) {
	  	a++;
  	}
  });
  return a
...
function quest4() {
  ...
  array.forEach((arrayContent) => {
	  ...
	  if(arrayContent[i].charCodeAt(0) >= "A".charCodeAt(0) && arrayContent[i].charCodeAt(0) <= "Z".charCodeAt(0)) {
	  	a++;
	  	break
  	  }
	}
...
function quest7() {
  let a = 0;
  array.forEach((arrayContent) => {
  	if(arrayContent.indexOf("@epenser") >= 0) {
	  	console.log(a)
	  	console.log(array[a])
  	}
  	a++;
...
function exo05() {
  ...
  console.log(quest1())
  console.log("Combien d'handles contiennent les 4 lettres du prénom 'Aude' à la suite (sans prendre en compte les majuscules) ?")
  console.log(quest2())
